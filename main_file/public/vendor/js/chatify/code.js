...
function sendigCard(message, id) {
 ...
 <div class="message-card mc-sender" data-id="` +
    ...
    `">
     <p>` +
    message +
    `<sub><span class="ti ti-clock"></span></sub></p>
 </div>
 `
  );
}
  ...
  if (fileType != "image") {
    return (
     ...
     <div class="attachment-preview">
         <span class="ti ti-x cancel"></span>
         <p style="padding:0px 30px;"><span class="ti ti-file"></span> ` +
      escapeHtml(fileName) +
      `</p>
     </div>
     `
    ...
    return (
     ...
     <div class="attachment-preview">
      ...
      imgURL +
      `');"></div>
         <p><span class="ti ti-file-image"></span> ` +
      escapeHtml(fileName) +
      `</p>
     </div>
     `
...
function errorMessageCard(id) {
  ...
  messagesContainer
    ...
    .prepend('<span class="ti ti-alert-triangle"></span>');
}
...
channel.bind("client-seen", function(data) {
  if (data.from_id == messenger.split("_")[1] && data.to_id == auth_id) {
    if (data.seen == true) {
      $(".message-time")
        .find(".fa-check")
        .before('<span class="ti ti-checks seen"></span> ');
      $(".message-time")
        .find(".fa-check")
        .remove();
